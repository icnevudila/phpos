...
function PhoneView(): JSX.Element {
  ...
  return (
    <div className="mx-auto w-full max-w-[190px] rounded-[34px] bg-white p-1.5 shadow-2xl ring-1 ring-slate-100">
        ...
        <div className="aspect-[9/19] p-4 text-[9px]">
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="flex gap-1.5">
              <button type="button" className="flex-1 rounded-lg bg-teal-600 py-1.5 text-[8px] font-bold text-white">
...
function TabletView(): JSX.Element {
  const { t } = useTranslation();
  return (
    <div className="mx-auto w-full max-w-[320px] rounded-[22px] bg-white p-2 shadow-2xl ring-1 ring-slate-100">
      <div className="overflow-hidden rounded-[16px] bg-white">
        <div className="aspect-[4/3] p-3 text-[9px]">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            ...
            <span className="rounded-full bg-teal-50 px-1.5 py-0.5 text-[8px] font-bold text-teal-700">
          ...
          <div className="mt-3 space-y-1.5">
            <div className="flex justify-center gap-0.5">
              {Array.from({ length: 16 }).map((_, i) => (
                <div
                  key={i}
                  className={`h-5 w-3 rounded-t border ${ i === 4 ? "border-rose-500 bg-rose-200" : i === 13 ? "border-teal-500 bg-teal-200" : "border-slate-300 bg-white" }`}
                />
              ))}
            </div>
            <div className="flex justify-center gap-0.5">
              {Array.from({ length: 16 }).map((_, i) => (
                <div
                  key={i}
                  className={`h-5 w-3 rounded-b border ${ i === 5 ? "border-sky-500 bg-sky-200" : "border-slate-300 bg-white" }`}
                />
              ))}
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-1">
            {[
              ...
              { l: t("landingPreview.devices.tabletLegendRct"), c: "bg-teal-500" },
          ...
          <div className="mt-3 rounded-lg bg-gradient-to-br from-teal-50 to-sky-50 p-2">
            <p className="text-[8px] font-bold uppercase text-teal-700">{t("landingPreview.devices.tabletNoteEyebrow")}</p>
...
function DesktopView(): JSX.Element {
  ...
  return (
    <div className="relative mx-auto max-w-sm">
      <div className="rounded-[14px] bg-white p-1.5 shadow-2xl ring-1 ring-slate-100">
        <div className="rounded-[8px] bg-[#f5f7f9] p-0.5">
            ...
            <div className="aspect-[16/10] p-3 text-[8px]">
              <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                ...
                <div className="flex gap-1">
                  ...
                  <span className="inline-flex items-center gap-0.5 rounded-full bg-teal-50 px-1.5 py-0.5 text-[6px] font-bold uppercase tracking-wider text-teal-700 ring-1 ring-teal-200">
                    <span className="h-[3px] w-[3px] rounded-full bg-teal-500" />
                    {t("landing.mockLive")}
                  </span>
                </div>
              </div>
              <div className="mt-2 grid grid-cols-4 gap-1.5">
                {[
                  { l: t("landingPreview.devices.deskToday"), v: "₱ 43K", c: "text-teal-600" },
                ...
                <div className="rounded bg-slate-50 p-2">
                  ...
                  <div className="mt-1.5 flex h-8 items-end gap-0.5">
                    {[55, 70, 62, 80, 75, 90, 68].map((h, i) => (
                      <div
                        key={i}
                        className="flex-1 rounded-t bg-gradient-to-t from-teal-400 to-sky-400"
                        style={{ height: `${h}%` }}
                      />
                    ))}
                  </div>
  ...
  return (
    <div>
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-slate-600">{t("landing.deviceSubtitle")}</p>
      </div>
        ...
        {DEVICES.map((d, i) => (
          ...
          >
            ...
            <div className="mt-6 flex flex-col items-center text-center">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{t(d.descKey)}</p>
            </div>
          </motion.div>
        ))}
      </div>

      <div className="mt-10 flex items-center justify-center gap-3 text-xs font-semibold text-slate-500">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 shadow-sm">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-3.5 w-3.5 text-teal-600">
