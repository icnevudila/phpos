    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-lg font-bold text-slate-900 dark:text-white">{t("landing.dayMockGreet")}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { l: t("landing.dayMockStatAppt"), v: "8", c: "text-emerald-600" },
      ...
      <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-sky-50 p-3 dark:from-emerald-950/40 dark:to-sky-950/40">
        <p className="text-[10px] font-bold uppercase text-emerald-700 dark:text-emerald-300">
          {t("landing.dayMockFirstLabel")}
        </p>
        <p className="mt-0.5 text-sm font-bold text-slate-900 dark:text-white">{t("landing.dayMockFirstDetail")}</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold text-slate-900 dark:text-white">Maria Santos</p>
          <p className="text-[10px] text-slate-500">{t("landing.dayMockPatientMeta")}</p>
        </div>
        <span className="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[9px] font-bold text-rose-700 ring-1 ring-rose-200 dark:bg-rose-950/40 dark:text-rose-300 dark:ring-rose-900">
      ...
      <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
        ...
        <p className="mt-0.5 text-sm font-semibold text-slate-900 dark:text-white">{t("landing.dayMockLastVisitDetail")}</p>
    ...
    <div>
      ...
      <div className="mt-3 space-y-2">
        <div className="flex justify-center gap-0.5">
          {[18, 17, 16, 15, 14, 13, 12, 11, 21, 22, 23, 24, 25, 26, 27, 28].map((n) => (
            <div
              key={n}
              className={`flex h-6 w-3.5 items-end justify-center rounded-t-md border text-[7px] ${
                highlight.has(n)
                  ? n === 15
        ...
        <div className="flex justify-center gap-0.5">
          {[48, 47, 46, 45, 44, 43, 42, 41, 31, 32, 33, 34, 35, 36, 37, 38].map((n) => (
            <div
              key={n}
              className="flex h-6 w-3.5 items-end justify-center rounded-b-md border border-slate-300 bg-white text-[7px] dark:border-slate-700 dark:bg-slate-900"
            >
              <span className="text-slate-400">{n}</span>
            </div>
          ))}
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-[9px] font-semibold">
        ...
        <span className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2 py-0.5 text-violet-700">
          <span className="h-1.5 w-1.5 rounded-full bg-violet-500" /> 26 · {t("landing.dayMockRootCanal")}
        </span>
      </div>
    </div>
  );
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm font-bold text-slate-900 dark:text-white">CLM-2026-0414-003</p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 p-3 text-xs dark:border-slate-700">
        <div className="flex justify-between">
          ...
          <span className="font-mono text-slate-800 dark:text-slate-200">₱ 10.500</span>
        </div>
      ...
      <div className="flex items-center gap-2 rounded-xl bg-emerald-50 p-2.5 ring-1 ring-emerald-200 dark:bg-emerald-950/40 dark:ring-emerald-900">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="h-4 w-4 text-emerald-600 dark:text-emerald-400">
          <path d="M20 6 9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        <span className="text-[11px] font-semibold text-emerald-800 dark:text-emerald-200">{t("landing.dayMockClaimSent")}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      {[
        ...
        return (
          <div key={r.n} className="rounded-lg bg-slate-50 p-2.5 dark:bg-slate-800">
            <div className="flex items-center justify-between text-xs">
              <span className="truncate font-semibold text-slate-800 dark:text-slate-200">{r.n}</span>
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">{t("landing.dayMockWeekly")}</p>
      </div>
      <div className="grid grid-cols-2 gap-2 text-center">
        {[
          ...
          <div key={s.l} className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800">
  ...
  return (
    <div>
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-slate-600 dark:text-slate-400">{t("landing.daySubtitle")}</p>
      </div>
      ...
      <div className="mx-auto mt-12 max-w-6xl space-y-4 lg:space-y-16">
        {scenes.map((s, i) => (
          <div
            ...
            data-scene={i}
            className={`relative grid gap-6 rounded-2xl border p-5 transition lg:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)] lg:items-start lg:gap-10 lg:p-6 ${
              i === active
          ...
          >
            ...
            <div className="min-w-0">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-base font-bold text-slate-900 sm:text-lg dark:text-white">{s.title}</h3>
