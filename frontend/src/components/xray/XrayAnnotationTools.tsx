    ...
    <div className={`space-y-3 rounded-lg bg-white p-4 shadow-sm ${className}`}>
      ...
      <div className="flex flex-wrap gap-2 border-b pb-3">
        <button
          ...
          className={`px-3 py-2 rounded text-sm font-medium ${ mode === 'view' ? 'bg-blue-500 text-white' : 'bg-slate-100 hover:bg-slate-200' }`}
        >
          View
        </button>
        <button
          onClick={() => setMode('brush')}
          className={`px-3 py-2 rounded text-sm font-medium flex items-center gap-1 ${ mode === 'brush' ? 'bg-blue-500 text-white' : 'bg-slate-100 hover:bg-slate-200' }`}
        >
          <Pencil size={16} /> Brush
        </button>
        <button
          onClick={() => setMode('ruler')}
          className={`px-3 py-2 rounded text-sm font-medium flex items-center gap-1 ${ mode === 'ruler' ? 'bg-blue-500 text-white' : 'bg-slate-100 hover:bg-slate-200' }`}
        >
          <Ruler size={16} /> Measure
        </button>
        <button
          onClick={() => setMode('angle')}
          className={`px-3 py-2 rounded text-sm font-medium flex items-center gap-1 ${ mode === 'angle' ? 'bg-blue-500 text-white' : 'bg-slate-100 hover:bg-slate-200' }`}
        >
          <Zap size={16} /> Angle
        </button>
        <button
