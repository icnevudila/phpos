    ...
    <section className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
           <div className="h-10 w-10 rounded-2xl bg-teal-500 flex items-center justify-center text-white shadow-lg shadow-teal-500/20">
              <Activity size={20} />
           </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="rounded-[2.5rem] bg-white p-8 shadow-xl ring-1 ring-slate-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 h-32 w-32 bg-teal-500/5 blur-3xl pointer-events-none" />
          ...
          {currentlyInClinic.length === 0 ? (
            ...
            <ul className="space-y-4">
              {currentlyInClinic.slice(0, 5).map((r) => (
                <li key={r.id} className="group relative flex items-center justify-between rounded-2xl bg-slate-50 p-4 transition-all hover:bg-white hover:shadow-lg hover:ring-1 hover:ring-slate-100">
          ...
          {nextAppointments.length === 0 ? (
            ...
            <ul className="space-y-4">
              {nextAppointments.slice(0, 4).map((r) => (
                <li key={r.id} className="flex items-center justify-between rounded-2xl bg-slate-50/50 p-4">
                  <div className="min-w-0">
                    <span className="block truncate text-sm font-bold text-slate-700">{r.patientName}</span>
        ...
        <div className="rounded-[2.5rem] bg-rose-500 p-8 shadow-2xl shadow-rose-500/20 ring-1 ring-rose-500/10 relative overflow-hidden">
