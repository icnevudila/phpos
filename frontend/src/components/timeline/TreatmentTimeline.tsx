...
const STATUS_CONFIG = {
  completed: { color: 'text-teal-500', bg: 'bg-teal-500/10', border: 'border-teal-500/20', icon: <CheckCircle2 size={16} /> },
...
export function TreatmentTimeline({
  ...
  return (
    <div className={`flex flex-col gap-8 ${className}`}>
      ...
      <header className="flex items-center justify-between rounded-[2.5rem] bg-white border border-slate-200 p-8 shadow-xl shadow-slate-200/40">
      ...
      <div className="relative pl-8 md:pl-24 space-y-12">
         {/* Vertical Path Line */}
         <div className="absolute left-10 md:left-28 top-0 bottom-0 w-px bg-gradient-to-b from-sky-500 via-slate-200 to-transparent" />
...
function JourneyPhaseNode({ phase, index, t }: any) {
  ...
  return (
    ...
    >
      ...
      {/* Status Orb */}
      <div className={`absolute -left-10 md:left-[-1.125rem] top-2 z-10 h-5 w-5 rounded-full border-4 border-slate-50 flex items-center justify-center ${config.bg} ${config.color} shadow-lg`}>
         {config.icon}
      </div>

      {/* Content Card */}
      <div className={`ml-4 md:ml-12 rounded-[2rem] border transition-all duration-300 ${ isExpanded ? 'bg-white shadow-2xl border-slate-200 p-8' : 'bg-slate-50 border-transparent hover:border-slate-200 p-6' }`}>
            ...
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="flex items-center gap-2">
                     <h4 className="text-lg font-black text-slate-900">{phase.title}</h4>
                   ...
                   {phase.treatments && (
                     <div className="flex flex-wrap gap-2">
                        {phase.treatments.map((t_item: string, i: number) => (
                          <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-100 text-[11px] font-black uppercase text-slate-500">
