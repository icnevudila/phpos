...
export function AdjustStockModal({ open, item, mode, onClose, onSubmit }: Props): JSX.Element | null {
  ...
  async function submit(e: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("pages.inventory.adjust.failed", { defaultValue: "Failed" }));
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-white/40 p-4">
      <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">
              {isIn ? t("pages.inventory.adjust.titleIn", { defaultValue: "Title In" }) : t("pages.inventory.adjust.titleOut", { defaultValue: "Title Out" })}
            </p>
        ...
        <form onSubmit={submit} className="space-y-4 px-6 py-5">
          ...
          <label className="block">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">
              {t("pages.inventory.adjust.reasonLabel", { defaultValue: "Reason Label" })}
            </span>
            <select
              value={reason}
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {busy
                ...
                : isIn
                  ? t("pages.inventory.adjust.submitIn", { defaultValue: "Submit In" })
                  : t("pages.inventory.adjust.submitOut", { defaultValue: "Submit Out" })}
            </button>
          </div>
        </form>
      </div>
