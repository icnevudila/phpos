...
export function PublicProfilePage(): JSX.Element {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f9] flex items-center justify-center p-6 font-sans">
      <div className="max-w-4xl w-full bg-white shadow-sm border border-slate-100 rounded-2xl overflow-hidden flex flex-col items-center py-12 px-6 relative">
        
        {/* HEADER SECTION (Persistent across flow) */}
        <header className="mb-12 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="h-12 w-12 text-teal-500">
            ...
            <span className="text-4xl font-light text-slate-400">
              <span className="text-teal-500 font-medium">My Dental</span> Clinic
            </span>
          </div>
        </header>
          ...
          {step === "profile" && (
            ...
            >
              ...
              <div className="w-full max-w-3xl grid md:grid-cols-[240px_1fr] gap-12 items-start">
                <div className="bg-teal-500 aspect-square flex items-center justify-center p-8 shadow-lg rounded-xl">
                ...
                <div className="space-y-8 pt-4">
                  <div className="grid gap-6">
                    {[
                      ...
                      <div key={item.label} className="grid grid-cols-[140px_1fr] items-center gap-4 border-b border-slate-100 pb-2">
              ...
              <div className="mt-10 flex flex-col items-center space-y-6">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input 
                    ...
                    className="mt-1 h-4 w-4 rounded border-teal-500 text-teal-500 focus:ring-teal-500" 
                  />
                ...
                <button 
                  ...
                  className={`px-10 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition shadow-lg ${ allowed ? "bg-teal-500 text-white shadow-teal-500/20 hover:bg-teal-600" : "bg-slate-100 text-slate-400 cursor-not-allowed" }`}
                >
                  Proceed
                </button>
              </div>
          ...
          {step === "questions" && (
            ...
            >
              ...
              <div className="space-y-8">
                {questions.map((q, i) => (
                  <div key={i} className="space-y-4 border-b border-slate-100 pb-6">
                    <p className="text-[13px] font-bold text-slate-800">{i + 1}. {q}</p>
                    <div className="flex gap-6">
                      {["Yes", "No"].map(opt => (
                        <label key={opt} className="flex items-center gap-2 cursor-pointer">
                          <div className="h-4 w-4 rounded-full border-2 border-teal-500 flex items-center justify-center">
                            <div className="h-2 w-2 rounded-full bg-teal-500 opacity-0 group-hover:opacity-100 transition" />
                          </div>
              ...
              <div className="mt-12 flex justify-center gap-4">
                <button onClick={() => setStep("intro")} className="bg-teal-500 text-white px-8 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest">Back</button>
                <button onClick={() => setStep("success")} className="bg-teal-500 text-white px-8 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                  Next <span>▶</span>
                </button>
              </div>
            </motion.div>
        ...
        {step === "success" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/40 backdrop-blur-sm p-6">
            <motion.div 
              initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-10 flex flex-col items-center text-center space-y-6"
            >
              <div className="h-24 w-24 rounded-full border-4 border-teal-50 flex items-center justify-center text-teal-500 mb-2">
              ...
              <div className="space-y-4 text-xs font-medium text-slate-500 leading-relaxed uppercase tracking-tight">
                <p>Your request for visit has been sent to <span className="text-teal-500 font-bold">DR. ASSSSSS GGGGGGG</span>. Please wait for the message or call from the clinic for updates!</p>
